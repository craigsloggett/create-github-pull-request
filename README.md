...
This produces the following `gh pr create` flags:

| Flag      | Value                                                                                                              |
| --------- | ------------------------------------------------------------------------------------------------------------------ |
| `--title` | chore(build): Update govulncheck to <version>                                                                      |
| `--body`  | Refreshes the pinned govulncheck version used by `make lint`.<br>Generated by the scheduled `update.yml` workflow. |
...
> [!CAUTION]
> If you provide an App token to this action so downstream workflows trigger on the PRs it opens, be aware that **those downstream workflows also have access to the token**. See [Security](#security) for ways to mitigate this risk.
