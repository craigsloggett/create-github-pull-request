...
The workflow's `permissions:` block must grant `contents: write` (to push the branch) and `pull-requests: write` (to open the PR). The default `github.token` is sufficient for both.
